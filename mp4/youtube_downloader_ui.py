    ...
    try:
        ...
        if resolution == '' and video_link == '':
            ...
            showerror(title='Error',
                      message='None is an invalid video resolution!!\n Please select a valid video resolution')
        else:
            try:
                def on_progress(stream, chunk, bytes_remaining):
                    total_size = stream.filesize

                    def get_formatted_size(t_size, factor=1024, suffix='B'):
                        for unit in ["", "K", "M", "G", "T", "P", "E", "Z"]:
                            if t_size < factor:
                                return f"{t_size:.2f}{unit}{suffix}"
                            t_size /= factor
                        return f"{t_size:.2f}Y{suffix}"
...
window.iconbitmap(window, 'youtubeicon.ico')
...
canvas.create_window(100, 315, window=search_resolution)
